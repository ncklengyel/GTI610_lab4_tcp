...
public class TcpServer implements Runnable {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			// Lorsqu'un client se connecte au créer une nouvelle instance du
	...
	@Override
	// Lorsque .start() est applé, ceci est la méthode qui est executer dans le
	// nouveau thread
	public void run() {
		...
		try {
			// instanciation du PrintWriter pour qu'il puisse envoyer des
			// données au client
			out = new PrintWriter(clientSocket.getOutputStream());

			// instanciation du BufferedReader pour qu'il puisse recevoir des
			// données du client
		...
		while (true) {
			...
			if (message != null) {
				...
				// On affiche à la console le message recu du client, son
				// adresse IP et son Port
				System.out.println("Message: " + message + " IP: " + clientSocket.getInetAddress() + " port: "
						+ clientSocket.getPort() + " Thread ID: " + Thread.currentThread().getId());
